...
class Main extends Component {
  ...
  componentDidMount() {
    if (this.props.loggedIn) {
      ...
      let rootRef = firebase.database().ref('users/' + this.props.user);
      rootRef.on('value', snapshot => {
        const result = snapshot.val();
        const list = Object.keys(result.watchlist)
        this.setState({
          myWatchList: list
        })
      });
  ...
  TagSelectHandler = (tag, tagKey) => {
    ...
    this.setState({search: updatedState, pageNo: 1});
  };
  ...
  componentDidUpdate(prevProps, prevState, SS) {
    if (prevState.orderDesc !== this.state.orderDesc) {
      ...
      this.setState({movieList: movieListOrder});
    }

    if (this.state.myWatchList !== prevState.myWatchList) {
      this.setState({searchError: false});
      const myWatchlist = this.state.myWatchList;
      let movList = [];
      myWatchlist.forEach((item) => {
        let rootRef = firebase.database().ref('movies').child(item);
        rootRef.on('value', snapshot => {
    ...
    if (prevState.search !== this.state.search) {
      ...
      rootRef.on('value', snapshot => {
        if (snapshot.val() !== undefined && snapshot.val() !== null) {
          ...
          this.setState({movieList: movList, oldMovieList: movList});
    ...
    if (prevState.search.theme !== this.state.search.theme) {
      ...
      if (this.state.search.theme.length > 0) {
        ...
        this.setState({movieList: this.state.oldMovieList});
      }
    }
    if (prevState.search.nature !== this.state.search.nature) {
      this.setState({searchError: false });
      if (this.state.search.nature !== null & this.state.search.nature !== "") {
        const filterList = [...this.state.oldMovieList];
        const filteredList = filterList.filter((item) => {
          let val = false;
          if (item.nature === this.state.search.nature){
            val = true
            return val;
          } else {
          return null;
    ...
    if (prevState.search.decade !== this.state.search.decade) {
      this.setState({searchError: false });
      if (this.state.search.decade !== null && this.state.search.decade !== "") {
        const filterList = [...this.state.oldMovieList];
        const filteredList = filterList.filter((item) => {
          let val = false;
          if (item.decade.toString() === this.state.search.decade){
            val = true
            return val;
          } else {
          return null;
  ...
  render() {
    ...
    if (!this.state.searchError) {
      ...
      movieResultsList = <div className='Container Centered'>
        <p>
          A movie matching these tags does not exist... It is now your responsibility to make it.</p>
      </div>
    }
